...
2. **准备 VM** - 检查/创建 VM，配置托管身份，安装 Docker
3. **配置权限** - 为 VM 托管身份分配 ACR 访问权限
4. **上传脚本** - 从 GitHub 下载部署脚本到 VM
5. **执行部署** - 在 VM 上运行部署脚本（使用托管身份访问 ACR）
6. **健康检查** - 验证所有服务正常运行

### 4. 托管身份优势
- **无密码认证** - VM 使用 Azure 托管身份访问 ACR
- **自动权限管理** - Azure 自动处理身份验证
- **更高安全性** - 无需存储或传递 ACR 密码
